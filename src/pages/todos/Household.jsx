...
export default function Household() {
  ...
  return (
    <div className="page">
      ...
      {activeTodos.length === 0 && (
        <div className="empty-state">No household to-dos — tap + to add one</div>
      )}
      ...
      {showModal && (
        <div className="modal-overlay" onClick={closeModal}>
          ...
          >
            ...
            <h2 className="modal-title">{editingId ? 'Edit to-do' : 'New household to-do'}</h2>
            ...
            <div className="form-row">
              ...
              <select className="form-select" value={form.subCategory} onChange={(e) => setForm({ ...form, subCategory: e.target.value })}>
                ...
                {SUBCATEGORIES_HOUSEHOLD.map((s) => <option key={s}>{s}</option>)}
              </select>
            </div>
