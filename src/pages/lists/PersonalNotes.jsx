...
export default function PersonalNotes() {
  ...
  const openAdd = () => {
    ...
    setModal({
      mode: 'edit',
      id: doc(collection(db, 'users', user.uid, 'personalNotes')).id,
      isNew: true,
      title: '',
      content: '',
      attachments: [],
  ...
  const save = async () => {
    ...
    const ref = doc(db, 'users', user.uid, 'personalNotes', modal.id)
    if (modal.isNew) {
        ...
        <div className="modal-overlay" onClick={close}>
          <div className="modal-sheet" onClick={(e) => e.stopPropagation()} style={{ maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            {modal.mode === 'view' ? (
              ...
              <>
                <h2 className="modal-title">{modal.isNew ? 'Add personal note' : 'Edit note'}</h2>
                <input
                ...
                <Attachments
                  ...
                  storagePrefix={user ? `users/${user.uid}/personalNotes/${modal.id}` : null}
                />
